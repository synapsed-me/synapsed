//! Decentralized Identity (DID) integration for anonymous payments
//! 
//! This module provides:
//! - DID-based user identification without linking to payment accounts
//! - DID rotation while maintaining subscription access
//! - Recovery mechanisms for lost DIDs
//! - Portable subscription proofs across DID changes

use chrono::{DateTime, Utc};
use did_key::{DIDKey, KeyMaterial, Secp256k1KeyPair};
use multibase::Base;
...
use crate::zkp::{AnonymousSubscription, SubscriptionTier, ZKProofEngine};
...
impl DIDManager {
    ...
    pub async fn verify_access(
        ...
        for (sub_id, did_hashes) in &self.subscription_mappings {
            if did_hashes.contains(&did_hash) {
                // In a real implementation, you'd verify the subscription through zkp_engine
                // For now, we'll assume it's valid if found
    ...
    pub async fn rotate_did(
        ...
        let now = Utc::now();
        let recovery_expires_at = now + chrono::Duration::days(30); // 30-day recovery window
        ...
        for (sub_id, did_hashes) in &mut self.subscription_mappings {
            if let Some(pos) = did_hashes.iter().position(|h| h == &old_did_hash) {
                ...
                // Notify ZKP engine of DID rotation
                let rotation_proof = self.generate_rotation_proof(old_did, new_did)?;
    ...
    pub async fn recover_access(
        ...
        for (sub_id, did_hashes) in &mut self.subscription_mappings {
            if let Some(pos) = did_hashes.iter().position(|h| h == &old_did_hash) {
                ...
                // Notify ZKP engine of recovery
                let recovery_proof = self.generate_recovery_proof(request)?;
                zkp_engine.rotate_did(sub_id, &request.old_did, &request.new_did, &recovery_proof).await?;
            }
        }
        ...
        let rotation = DIDRotation {
            ...
            recovery_expires_at: request.timestamp + chrono::Duration::days(7), // Shorter window for recovery
    ...
    pub async fn generate_portable_proof(
        ...
        // Generate ZK proof of subscription validity
        let validity_proof = zkp_engine.generate_subscription_proof(
            subscription_id,
            SubscriptionTier::Free, // Minimum tier for portability
            "portable_proof",
        ).await?;
        ...
        Ok(PortableSubscriptionProof {
            subscription_id: subscription_id.to_string(),
            validity_proof: validity_proof.validity_proof,
            did_commitment,
            timestamp: now,
            expires_at: now + chrono::Duration::hours(24), // 24-hour portability window
    ...
    fn verify_did_signature(&self, did: &str, signature: &[u8], message: &str) -> PaymentResult<bool> {
        ...
        // In production, this would properly verify the DID signature
    ...
    fn generate_rotation_proof(&self, old_did: &str, new_did: &str) -> PaymentResult<Vec<u8>> {
        // Generate proof that the same entity controls both DIDs
        let proof_data = format!("rotation:{}->{}", old_did, new_did);
    ...
    fn generate_recovery_proof(&self, request: &DIDRecoveryRequest) -> PaymentResult<Vec<u8>> {
        let proof_data = format!("recovery:{}->{}:{:?}", 
            request.old_did, request.new_did, request.recovery_method);
        Ok(proof_data.as_bytes().to_vec())
    }
...
mod tests {
    ...
    async fn test_cleanup_expired() {
        ...
        let mut session = DIDSession {
            did: "did:key:test".to_string(),
            session_id: "expired_session".to_string(),
            created_at: past_time,
            expires_at: past_time + chrono::Duration::minutes(30),
